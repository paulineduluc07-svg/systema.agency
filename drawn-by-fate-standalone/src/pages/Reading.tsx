...
import { FateLayout, FateDivider } from "@/components/FateLayout";
import { FlippableCard } from "@/components/TarotCardSVG";
        ...
        {reading.cardReadings.map((cr, i) => (
          ...
          >
            <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
              ...
              <div>
                <p style={{ color: "#CC0000", fontSize: 12, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>
                  {cards[i]?.nameFr}
                </p>
...
export default function Reading() {
  ...
  async function handleReveal() {
    ...
    // Fallback reading (no API required)
    await new Promise((r) => setTimeout(r, 1200));
    setReading({
      cardReadings: chosenCards.map((c) => ({
        cardName: c.nameFr,
        interpretation: `La carte ${c.nameFr} porte le message de : ${c.upright}. Ses mots-clés — ${c.keywords.join(", ")} — résonnent dans ce moment. Méditez sur son message avec soin et laissez-le guider votre intuition.`,
      })),
      overallReading:
        "Les cartes ont parlé. Leur message vous appartient — lisez-le avec le cœur ouvert et laissez le destin vous guider sur le chemin qui vous attend.",
    });
    setIsLoading(false);
    setTimeout(() => {
      ...
      <div style={{ padding: "20px 24px 60px", maxWidth: 860, margin: "0 auto" }}>
        ...
        {!isShuffling && (
          <motion.div
            ...
            style={{ display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "center", marginBottom: 32 }}
          >
            {deck.map((card, i) => (
              <motion.div
                key={card.id}
